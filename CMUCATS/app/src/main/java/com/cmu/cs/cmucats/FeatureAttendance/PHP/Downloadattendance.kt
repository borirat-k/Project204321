...
class JsonDownloadDetailSchedule(var context: Context, private var phpUrl:String, var stu_id:Int) : AsyncTask<Void, Void, String>() {
    ...
    private fun download():String{
        ...
        try{
            ...
            var data: String = URLEncoder.encode("stu_id", "UTF-8") + "=" + URLEncoder.encode(stu_id.toString(), "UTF-8")
            writer.write(data)
            writer.flush()
            writer.close()
            ops.close()
    ...
    override fun onPostExecute(jsonData: String) {
        ...
        if(jsonData!!.startsWith("URL ERROR")){
            ...
            parserattendance(context,jsonData).execute()
        }
    }
