...
class ParserSubjectDetail(var context: Context, private var jsonData: String) : AsyncTask<Void, Void, Boolean>() {

    private lateinit var mView:CatLoadingView
    companion object {
        var coursedetailL = ArrayList<Coursedetail>()
    }
    ...
    private fun parse(): Boolean {
        try {
            ...
            for (i in 0 until ja.length()) {
                ...
                var c = Coursedetail(idCourse, startTime, stopTime, firstDay, secondDay)
                coursedetailL.add(c)

                println( "$idCourse $startTime $stopTime $firstDay $secondDay")
            }
...
class Coursedetail(val idCourse: String?, val startTime: String?, val stopTime: String?, val firstDay: String?, val secondDay: String?) {}